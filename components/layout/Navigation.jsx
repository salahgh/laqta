...
export const Navigation = ({className = ""}) => {
    ...
    return (
        <nav className={`w-full flex ${className} h-[105px] items-center justify-center`}>
            {/* Logo Section */}
            <div style={{width : 1326 , height : 62.5}} className={'flex items-center justify-between '}>

                    <Logo/>

                {/* Navigation Links - Hidden on mobile */}
                <div className={'flex justify-center h-full'}>
                    <div className="lg:flex items-center gap-11 border-[1px] rounded-full px-14 "
                    ...
                    >
                        {navItems.map((item) => (
                            ...
                            >
                                <span className={'text-body-xl font-medium'}>{item.label}</span>
                            </NavLink>
                        ))}
                    </div>
